...
-- @email englekk@naver.com, englekk@apollocation.com
-- "I belong to Jesus."
----------------------------------------

--[[
	-- @Example
	local ScrollView = require("com.wonhada.controls.ScrollView")
...
ScrollView.create = function (_W, _H)
	-- 로컬 변수 정의
	local firstX, firstY, firstXScale, firstYScale, bg
					...
					elseif eventTime - group.content.tapTime < 200 then
						...
						if group.doubleClickZoomEnabled then
							...
							if group.content.xScale == group.content.xScaleStart and group.content.yScale == group.content.yScaleStart then
								...
								transition.to( group.content, { time=300, transition=easing.outQuad, xScale = tw, yScale = th, x=tx, y=ty } )
							end
						end
					end
				else
